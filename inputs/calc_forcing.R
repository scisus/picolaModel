...
here::i_am("inputs/calc_forcing.R")
...
pnwnamet_adj <- picolaDataClimate::picola_PNWNAmet_adjusted %>%
  select(Date, Site, mean_temp_corrected) %>%
  rename(mean_temp = mean_temp_corrected) %>%
  mutate(DoY = yday(Date), Year = lubridate::year(Date))

typical_year <- picolaDataClimate::picola_typical_ts %>% 
  rename(mean_temp = mean_mean_temp)
normals <- picolaDataClimate::picola_normal_daily 
...
write.csv(dailyforc_1945_2012, here::here("inputs/forcing/dailyforc_1945_2012.csv"), row.names = FALSE)
write.csv(typical_year_forc, here::here("inputs/forcing/typical_year_forc.csv"), row.names = FALSE)
write.csv(normal_forc, here::here("inputs/forcing/normalforc_1921-2100.csv"), row.names = FALSE)
